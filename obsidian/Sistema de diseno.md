...
- **Hero por tema:** si existe `src/assets/atmosfera/hero-ruta-diurna.jpg` se renderizan dos fotos (`.solo-oscuro` / `.solo-claro`), la del tema por defecto con prioridad; comparten el mapa de profundidad porque es procedural. Sin ella, en claro se ve la nocturna con brillo 1. `.btn-vial` vive una sola vez en `global.css`.
