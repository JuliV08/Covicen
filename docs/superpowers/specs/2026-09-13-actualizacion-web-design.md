...
Cada formulario muestra junto al botón "Acuse en 24 hs · Respuesta en 5 días hábiles". Sin canal (`whatsapp` y correo `null`): deshabilitados con el aviso "Los formularios se habilitan con la toma de posesión, el 5 de octubre de 2026. Mientras, el 140 atiende emergencias las 24 horas". `capacidades.ticketingReclamos` **es a futuro**: cuando exista el CRM habrá que **implementar el POST en `Formulario.astro`** (y el número de gestión en la respuesta). Hoy `Formulario.astro` no lee el flag: prenderlo solo hace desaparecer los dos `HuecoCapacidad` de "Seguimiento de reclamos" (`/contacto/` y `/servicios/`) y los formularios siguen igual.
...
> **Corregido en la ejecución (2026-09-14):** los 12 KB gz de fuente no alcanzaron y el tope de `tests/presupuesto.test.ts` **se subió a 16 KB** (16.384 bytes). Al cierre van **16.244 bytes = 15,86 KB gz** (140 de margen). El de emitido no se tocó: 30 KB gz, con 5,4 KB reales. La decisión completa está en el plan (Global Constraints) y en `obsidian/Decisiones de arquitectura.md`.
