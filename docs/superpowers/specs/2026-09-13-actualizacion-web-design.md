...
- **Sin destello:** un `<script is:inline>` en el `<head>` de `Base.astro` (antes de cualquier estilo que dependa del tema) lee `localStorage.tema` (con `try/catch`), cae a `TEMA_POR_DEFECTO`, y fija `data-tema` y `theme-color` antes del primer pintado. Como el sitio usa `<ClientRouter />`, el mismo snippet se reaplica en `astro:after-swap` (los atributos de `<html>` se reemplazan en cada navegación).
- **Interruptor:** botón en la barra superior (ícono sol/luna de lucide), `aria-label="Cambiar a tema claro"` / `"…oscuro"`, `aria-pressed`. Guarda en `localStorage.tema` y emite `document.dispatchEvent(new CustomEvent('tema:cambio'))` para que el canvas de la grilla y el parallax reaccionen. En celular vive dentro del menú.
