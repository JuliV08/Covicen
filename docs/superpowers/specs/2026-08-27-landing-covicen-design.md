...
> **Segunda pasada de Juli (2026-08-27, tarde):** pidió tres referencias de 21st.dev (Binaural Glow Feature Card, Flip Card, Kinetic Grid) — excepción explícita a su regla de no extraer de ahí. Se **portaron a Astro/CSS/vanilla** (React sigue afuera) a partir del bundle público del preview: `.tarjeta` (borde de gradiente animado + glow, variante `vial`) reemplaza a los esquineros en todo el sitio; la tarifa de la Home es una sola card con flip 3D; las secciones oscuras llevan una grilla cinética en canvas; el hero termina en un divisor con ornamento y el tramo va dentro de un panel. Presupuesto de JS de animación: 9 KB gz.
>
> **Foto del hero (2026-08-27, tarde):** con la fotografía real (`hero-ruta-nocturna.jpg`), las luces sintéticas encima se leían como glitch y se quitaron. En su lugar, **parallax 2.5D**: canvas WebGL que desplaza la foto según un mapa de profundidad procedural (`scripts/generar-profundidad.py`), con avance automático, mouse y scroll; solo desktop con hover; en mobile, dolly CSS. Video con IA quedó descartado por ahora: Sora cerró (abril 2026) y Flow con Google AI Pro conserva marca visible según fuentes de terceros.
>
