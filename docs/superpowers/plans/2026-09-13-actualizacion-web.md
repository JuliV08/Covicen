...
- **Presupuestos:** JS emitido ≤ 30 KB gz (`scripts/verificar.ts`), scripts fuente ≤ 12 KB gz (`tests/presupuesto.test.ts`), `og.png` ≤ 300 KB. Se mantienen.
  - **Corregido otra vez (2026-09-15): el tope de fuente volvió a 14 KB porque cambió LO QUE SE MIDE.** `tests/presupuesto.test.ts` saca los comentarios antes de pesar: no se descargan (el bundler los tira) y habían llegado a ser el 25 % del total, o sea que el tope se subió dos veces por documentar. Con código solo van 12,6 KB. El gate real sigue siendo el JS emitido (30 KB gz, `verificar.ts`): van 5,4.
...
- **Cambios pedidos DESPUÉS de ejecutar este plan (2026-09-15, sobre el sitio ya publicado).** El plan de abajo queda como quedó: es el registro de lo que se hizo, no la descripción del sitio de hoy. Lo que dejó de valer:
  - **El hero no rota.** La Fase 5 construyó un carrusel que alternaba la portada con las novedades `destacada: true`; Juli lo dio de baja entero. Con él se fueron `src/scripts/rotacion.ts`, su test y el `scroll-snap` que pedía la spec §10.3. `destacada: true` quedó sin uso.
  - **Los avisos no están en la barra superior.** Pasaron a una cinta que desfila (`src/components/Marquesina.astro`), en el borde de abajo del hero en la home y arriba del contenido en el resto. La barra superior quedó con los accesos y **solo en escritorio**; `--alto-header` baja a 4,5rem abajo de 1024 px.
  - **En el tema claro, las tarjetas y los paneles de contenido van en oscuro** (`html[data-tema="claro"] :is(.tarjeta, .bloque-oscuro)`, la misma regla de `.zona-noche`). El fondo de la página sigue claro; el cromo y los campos de formulario también.
