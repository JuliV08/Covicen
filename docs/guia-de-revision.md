...
## Home `/`
- Hero **con la foto** (`src/assets/atmosfera/hero-ruta-nocturna.jpg`): la autopista real de fondo, oscurecida, con Ken Burns lento y parallax; encima, una luz amarilla que recorre la línea del cantero, una blanca por el borde derecho, luces traseras rojas que se alejan por los dos carriles y faros blancos que vienen por la calzada opuesta, detrás del guardarraíl. **Chequeá que las luces caigan sobre los carriles de la foto**: están medidas para esa imagen; si la reemplazás por otra, hay que remedir los paths en `HeroRuta.astro` (variante `foto`). Sin foto, vuelve la calzada vectorial en perspectiva. Una luz suave sigue al cursor. Eyebrow, título, texto, botones y cuenta regresiva entran escalonados con desenfoque; la cuenta regresiva corre por segundos.
